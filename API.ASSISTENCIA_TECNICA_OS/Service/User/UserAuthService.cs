...
namespace API.ASSISTENCIA_TECNICA_OS.Service.User
{
    public class UserAuthService : IUserService
    {
        ...
        {
            var request = new HttpRequestMessage(HttpMethod.Get, "http://172.26.160.1:8080/api/v1/auth/login");

            using var cliente = new HttpClient();

            var responseApiAuth = await cliente.GetAsync("http://192.168.1.166:8080/api/v1/auth/login");
